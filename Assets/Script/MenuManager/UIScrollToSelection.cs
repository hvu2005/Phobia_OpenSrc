...
namespace UnityEngine.UI.Extensions
{
    ...
    public class UIScrollToSelection : MonoBehaviour
    {
        ...
        protected GameObject CurrentSelectedGameObject
        {
            get 
            { 
                if(EventSystem.current.currentSelectedGameObject.transform.parent)
                {
        ...
        private void UpdateVerticalScrollPosition(RectTransform selection)
        {
            ...
            float normalizePosition = TargetScrollRect.verticalNormalizedPosition + (offlimitsValue/ LayoutListGroup.rect.height);

            if(normalizePosition < 0)
            {
                //scrolling up
                normalizePosition -= (Mathf.Abs(offlimitsValue) / LayoutListGroup.rect.height);
            }
            else if (normalizePosition > 0)
            {
                //scrolling down
                normalizePosition += (offlimitsValue / LayoutListGroup.rect.height);
            }

            //clamp the normalized position to ensure it stays within valid bounds [0,1]
            normalizePosition = Mathf.Clamp01(normalizePosition);

            // smoothly move the target scroll rect to the new position
            TargetScrollRect.verticalNormalizedPosition = Mathf.SmoothStep(TargetScrollRect.verticalNormalizedPosition, normalizePosition, Time.unscaledDeltaTime * scrollSpeed);
        }
